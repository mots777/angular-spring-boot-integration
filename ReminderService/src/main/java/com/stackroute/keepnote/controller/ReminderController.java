...
public class ReminderController {
	...
	public ResponseEntity<Reminder> createReminder(@RequestBody Reminder reminder, final HttpSession session) {

		Reminder addedReminder;
		reminder.setReminderCreationDate(Calendar.getInstance().getTime());
		try {
			addedReminder = this.reminderService.createReminder(reminder);
	...
	public ResponseEntity<Void> deleteReminder(@PathVariable("id") String reminderId, final HttpSession session) {
...
//		}
	...
	public ResponseEntity<Reminder> updateReminder(@PathVariable("id") String reminderId, @RequestBody Reminder reminder,
		...
		Reminder reminderUpdt = null;
		try {
			this.reminderService.getReminderById(reminderId);
			reminderUpdt = this.reminderService.updateReminder(reminder, reminderId);
		} catch (ReminderNotFoundException e1) {
			return new ResponseEntity<Reminder>(HttpStatus.NOT_FOUND);
		}

		if (null == reminderUpdt) {
			return new ResponseEntity<Reminder>(HttpStatus.CONFLICT);
		}
		
		return new ResponseEntity<Reminder>(reminderUpdt, HttpStatus.OK);
	...
	public ResponseEntity<Reminder> getReminderById(@PathVariable("id") String reminderId) {
//		if (!isUserLoggedIn(session)) {
...
//		}
	...
	public ResponseEntity<List<Reminder>> getAllReminders() {
		...
		final List<Reminder> userReminderLst = this.reminderService.getAllReminders();

		return new ResponseEntity<List<Reminder>>(userReminderLst, HttpStatus.OK);
